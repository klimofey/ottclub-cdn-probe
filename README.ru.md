...
**Или пассивно.** С `OBSERVE_ONLY` контейнер вообще не трогает выбор CDN: он
меряет тот, который на аккаунте уже стоит. Ничего не перебрасывается, смотреть
можно круглосуточно. См. раздел
[Две копии: одна рулит, вторая смотрит](#две-копии-одна-рулит-вторая-смотрит).
...
| `OBSERVE_ONLY` | `false` | Мерить тот CDN, который стоит, и никогда не переключать |
...
| `SETTLE_SECONDS` | `210` | Сколько ждать вступления CDN в силу перед замером |
...
## Две копии: одна рулит, вторая смотрит

`OBSERVE_ONLY=true` превращает копию в наблюдателя. Круг перестаёт быть
обходом всех CDN и становится одним замером того CDN, который на аккаунте
уже стоит; в выбор CDN не пишется ничего — ни кругом, ни `AUTO_APPLY`. Как
часто смотреть, решает `ROUND_PAUSE`: пассивный круг занимает минуты, а не
часы, так что `30m` — разумный интервал.
...
Ценно это двумя вещами. Такую копию можно направить на аккаунт, с которого
реально смотришь, — ничего не перебрасывается. И она позволяет со второй
машины наблюдать за аккаунтом, который переключает другая копия (например, в
докере где-то ещё): две копии, дёргающие выбор CDN, мешали бы друг другу, и
каждая записывала бы чужой CDN под своим именем.

CDN читается из панели до замера и ещё раз после. Если другая копия успела
переключить аккаунт в промежутке, замер накрыл сразу два CDN — он
выбрасывается, а не приписывается одному из них. Круг с неверной меткой хуже
пропущенного: он попадает в медиану и агитирует за не тот CDN всё время,
пока хранится.

Наблюдателю стоит дать собственный том с данными, если хочешь держать его
цифры отдельно от активной копии. Один том на двоих — журналы сольются, и это
обычно не то, что нужно: замеры наблюдателя описывают тот CDN, который в этот
момент тестировала другая копия.
...
## Ожидание вступления в силу

Новому CDN даётся **3.5 минуты**, прежде чем что-либо измеряется. Панель
обещает 5–10; замер раньше ловит смесь уходящего CDN и приходящего, и смесь
записывается как вина приходящего. Это не гипотеза: так один CDN показал
0.86x, имея в пуле чужую площадку, — в то время как его владелец смотрел на
нём без единой заминки.

Ожидание почти бесплатно. Переключаться всё равно разрешено раз в пять минут,
поэтому выдержка 3.5 минуты плюс замер на две заканчиваются ровно к моменту,
когда следующее переключение становится возможным. Ждать меньше — не значит
пройти круг быстрее, значит лишь получить неверные числа.

Смена пула эджей по-прежнему фиксируется, но как признак достоверности, а не
как сигнал начинать замер: смена пула на одном канале не означает, что
переключение доехало везде.
